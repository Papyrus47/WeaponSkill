    ...
    public class BowJustShoot : BasicBowsSkill
    {
        ...
        public virtual void Shoot()
        {
            ...
            for (int i = 0; i < Count; i++)
            {
                ...
                if (player.HeldItem?.ModItem?.Shoot(player, player.GetSource_ItemUse_WithPotentialAmmo(player.HeldItem, ammoType) as EntitySource_ItemUse_WithAmmo, Projectile.Center, Projectile.velocity.RotatedByRandom(MathHelper.PiOver4 * 0.35f) * BowsProj.SpawnItem.shootSpeed * speed, shootType, (int)((Projectile.damage + dmg) * BowsProj.ChannelLevel * 0.9f), Projectile.knockBack + kn) == false)
                    return;
